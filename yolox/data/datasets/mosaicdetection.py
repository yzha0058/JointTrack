...
class MosaicDetection(Dataset):
    ...
    def __getitem__(self, idx):
        if self.enable_mosaic:
            ...
            if len(mosaic_labels):
                ...
                # mosaic_labels = mosaic_labels[mosaic_labels[:, 6] < 2 * input_w]
                # mosaic_labels = mosaic_labels[mosaic_labels[:, 8] > 0]
                # mosaic_labels = mosaic_labels[mosaic_labels[:, 7] < 2 * input_h]
                # mosaic_labels = mosaic_labels[mosaic_labels[:, 9] > 0]
    ...
    def mixup(self, origin_img, origin_labels, input_dim):
        ...
        cp_body_bboxes_origin_np = adjust_box_anns(
        ...
        if FLIP:
            ...
            cp_body_bboxes_origin_np[:, 0::2] = (
                    origin_w - cp_body_bboxes_origin_np[:, 0::2][:, ::-1]
            )
        cp_bboxes_transformed_np = cp_bboxes_origin_np.copy()
        cp_body_bboxes_transformed_np = cp_body_bboxes_origin_np.copy()
        '''
        ...
        cp_body_bboxes_transformed_np[:, 0::2] = cp_body_bboxes_transformed_np[:, 0::2] - x_offset
        cp_body_bboxes_transformed_np[:, 1::2] = cp_body_bboxes_transformed_np[:, 1::2] - y_offset
        ...
        if keep_list.sum() >= 1.0:
            ...
            body_box_labels = cp_body_bboxes_transformed_np[keep_list]
            labels = np.hstack((box_labels, cls_labels, id_labels, body_box_labels))
            # remove outside bbox
            ...
            #     start_point = (x, y)
            #     end_point = (x2, y2)
            #     origin_img = cv2.rectangle(origin_img, start_point , end_point , (0,255,0), 2)
            #     x = int(label[6])
            #     y = int(label[7])
            #     x2 = int(label[8])
            #     y2 = int(label[9])
            #     start_point = (x, y)
            #     end_point = (x2, y2)
            #     origin_img = cv2.rectangle(origin_img, start_point , end_point, (255, 0, 0), 2)
            #
